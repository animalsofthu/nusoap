                ...
                if ($valueType == 'arraySimple' || preg_match('/^ArrayOf/', $type)) {
                    ...
                    if (is_array($val) && count($val) > 0) {
                        foreach ($val as $v) {
                            ...
                            $xml .= $this->serialize_val($v, $use == 'literal' ? preg_replace('%s$%i', '', $name) : 'item', false, false, false, false, $use);
                            ++$i;
                        }
                    ...
                    if (isset($val['@XMLElement'])) {
                      $itemName = $val['@XMLElement'];
                      unset($val['@XMLElement']);
                    }
                    else {
                      unset($itemName);
                    }
                    foreach ($val as $k => $v) {
                        ...
                        if ($type == 'Map' && $type_ns == 'http://xml.apache.org/xml-soap') {
                            ...
                            $xml .= $this->serialize_val($v, $itemName ?? $k, false, false, false, false, $use);
                        }
                    }
                    $xml .= "</$name>";
                }
...
class nusoap_server extends nusoap_base
{
    ...
    function invoke_method()
    {
        ...
        if (isset($this->methods[$this->methodname])) {
          if (!is_callable($this->methods[$this->methodname])) {
            $this->debug("in invoke_method, function '{$this->methods[$this->methodname]}' not found!");
            $this->result = 'fault: method not found';
            $this->fault('SOAP-ENV:Server', "function '{$this->methods[$this->methodname]}' not defined in service('$try_class' '$delim')");
            return;
          }
        }
        else
        ...
        if (!function_exists('call_user_func_array')) {
            ...
            $call_arg = $this->methods[$this->methodname] ?? $call_arg;
            if (is_array($this->methodparams)) {
                $params = $this->methodparams;

                if (is_array($call_arg)) {
                  $ref = new \ReflectionMethod(get_class($call_arg[0]), $call_arg[1]);
                }
                else {
                  $ref = new \ReflectionFunction($call_arg);
                }

                $args = [];
                foreach ($ref->getParameters() as $parameter) {
                  if (isset($params[$parameter->getName()])){
                    $arg = $params[$parameter->getName()];
                  }
                  elseif ($parameter->isOptional()) {
                    $arg = $parameter->getDefaultValue();
                  }
                  else {
                    $message = sprintf('Argument "%s" is required!', $parameter->getName());
                    // throw new InvalidArgumentException($message);
                    $this->fault('SOAP-ENV:Client', $message);
                    return;
                  }

                  $args[] = $arg;
                  unset($params[$parameter->getName()]);
                  ...
                  if ($parameter->hasType()) {
                    ...
                    if ($class) {
                      ...
                      if (!$arg instanceof $className) {
                        $message = sprintf('Argument "%s" must be an instance of "%s", "%s" given!', $parameter->getName(), $className, is_object($arg) ? get_class($arg) : gettype($arg));
                        // throw new TypeError($message);
                        $this->fault('SOAP-ENV:Client', $message);
                        return;
                      }
                    }
                    elseif (gettype($arg) != $parameter->getType() && (!$parameter->isOptional() || NULL !== $arg)) {
                      $message = sprintf('Argument "%s" must be a %s, %s given!', $parameter->getName(), $parameter->getType(), is_object($arg) ? get_class($arg) : gettype($arg));
                      // throw new TypeError($message);
                      $this->fault('SOAP-ENV:Client', $message);
                      return;
                    }
                ...
                $this->methodreturn = call_user_func_array($call_arg, $args);
                /*
                try {
                  $this->methodreturn = call_user_func_array($call_arg, $args);
                } catch (\Throwable $exception){
                  $message = $exception->getMessage();
                  $message = preg_replace('%, called in.*?$%s', '', $message);
     ...
     * @access   public
     */
    function register(string $name, $in = array(), $out = array(), $namespace = false, $soapaction = false, $style = false, $use = false, $documentation = '', $encodingStyle = '')
    {
        global $HTTP_SERVER_VARS;
